...
// Function to convert image to base64 with high quality and preserve aspect ratio
const getImageAsBase64 = async (imagePath: string): Promise<{ dataUrl: string; width: number; height: number }> => {
...
export const generateSalarySlipPDF = async (salarySlip: SalarySlip): Promise<jsPDF> => {
    ...
    // Add Logo on the left, positioned down to align with text
    let logoHeight = 0;
    let logoWidth = 0;
    try {
        const logoData = await getImageAsBase64(stsLogo);
        const aspectRatio = logoData.width / logoData.height;
        logoWidth = 35; // Increased size
        logoHeight = logoWidth / aspectRatio;
        // Position logo down by 5mm to align with company name
        const logoY = y + 5;
        doc.addImage(logoData.dataUrl, 'PNG', margin, logoY, logoWidth, logoHeight);
    } catch (error) {
    ...
    doc.text('SYNCALL TECHNOLOGY SOLUTIONS PRIVATE LIMITED', textStartX, headerStartY);

    let textY = headerStartY + 6;
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.text('U72200TG2014PTC093379', textStartX, textY);
    textY += 5;

    doc.setFontSize(8);
    doc.text('H No 4-86, Plot No 8, Road No 2, Ganesh Nagar', textStartX, textY);
    textY += 4;
    doc.text('Narapally, K.V Rangareddy, Hyderabad, Telangana-500088', textStartX, textY);

    // Move y down after header section
    y = Math.max(y + logoHeight + 5, textY) + 10;
    ...
    const deductions = [
        ...
        ['INCOME TAX', formatCurrency(salarySlip.income_tax)],
    ];
...
export const downloadSalarySlipPDF = async (salarySlip: SalarySlip): Promise<void> => {
    const doc = await generateSalarySlipPDF(salarySlip);
    const monthName = MONTHS.find(m => m.value === salarySlip.month)?.label || 'Unknown';
